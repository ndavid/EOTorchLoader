...
def load_geo_img_dir(img_dir: Path, img_suffix_list=[".tif", ".jp2"]) -> pd.DataFrame:
    ...
    are written in a pandas Dataframe with one row by image and with columns :
        ...
        * "transform" : full affine transform to convert from pixel coordinate to grund coordinate.
            follow the rasterio convention with
            transform =
            (resolution/scale x, rotation row/x, upper_left/origin x,
            rotation col/y, resolution/scale y, upper_left/origin y)

    Parameters
    ----------
    img_dir : Path
        Path of the directory where images will be search
    img_suffix_list : List[str]
        list of image extension to filter files in img_dir.
